...
target = '''                        <label style="display:block; margin-bottom:5px; color:#555;">口座名義 (カタカナ)</label>
...
replacement = '''                        <label style="display:block; margin-bottom:5px; color:#555;">口座名義 (カタカナ)</label>
                        <input type="text" id="bank-holder" class="swal2-input" placeholder="例: ヤマダ タロウ" style="width:80%; margin:0 0 15px 0;">
                ...
                if (result.isConfirmed) {
                    ...
                    fetch(`${API_BASE}/api/creator/bank`, {
                        ...
                        if(resData.success) {
                            Swal.fire('保存しました', resData.message, 'success');
                        } else {
...
if target in text:
    ...
    print('creator target not found')

with codecs.open('server_retail_dist.js', 'r', 'utf-8') as f:
    srv = f.read()

starget = '''app.post('/api/creator/bank', (req, res) => {
...
sreplacement = '''app.post('/api/creator/bank', async (req, res) => {
...
if starget in srv:
    srv = srv.replace(starget, sreplacement)
