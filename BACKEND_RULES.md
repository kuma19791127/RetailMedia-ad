...
## 🚨 必須ルール6: クラウド本番環境におけるパッケージ互換性とGLIBC制限 (Cloud Platform Compatibility)

ネイティブC++バイナリを伴うNode.jsのモジュール（特に `sqlite3` や `fluent-ffmpeg` 等）を導入またはバージョン更新する際は、クラウド本番環境（AWS App RunnerやECS等のLinuxコンテナ環境）のライブラリ互換性に十分注意してください。

1. **GLIBC（C標準ライブラリ）のバージョン制限への配慮**
   - 開発環境（Windows/macOS等）の最新OSや、新しいOSバージョンのビルド環境で動作したとしても、本番環境のベースOSが古い場合、バイナリロード時に `Error: /lib64/libm.so.6: version 'GLIBC_2.38' not found` 等のDLOPENエラーが発生してコンテナの起動に失敗（Exit 1）します。
2. **本番環境と親和性の高い枯れたバージョンの使用**
   - 本番でC++ネイティブバイナリモジュールを使用する場合は、プリコンパイル済みの実行用バイナリの要求GLIBCバージョンが低い、実績のある枯れたバージョン（例: `sqlite3` における `5.1.7` 等）を明示的に選択し、環境間のミスマッチを防止すること。
3. **不要なパッケージ読み込み（Require）の遅延化または排除**
   - ローカル開発のみで使用し、本番環境のデータベース（RDS/PostgreSQLなど）稼働時には不要となるモジュール（SQLite等）は、本番環境の起動ルートにおいて無条件で `require` しないよう条件分岐で動的インポート化するか、読み込み自体を完全に分離すること。
