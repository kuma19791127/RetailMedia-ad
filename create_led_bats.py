...
chcp 65001 >nul
echo =========================================================
echo Retail Media サイネージ自動起動セットアップ ({name})
echo =========================================================
echo.
echo このパソコンを「{name}専用」のサイネージとして設定します。
echo 設定が完了すると、次回以降は「電源を入れるだけ」で
echo 全自動で動画配信がスタートするようになります。
echo.
echo 設定中...

set "TARGET_URL=https://retail-ad.com/signage_player.html{param}"
set "STARTUP_FOLDER=%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup"
set "SHORTCUT_NAME=RetailAd_Signage_{id}.lnk"
...
configs = [
    {"id": "fish", "name": "鮮魚対面ガラス (1000x300)", "param": "?panel=fish", "file": "setup_led_fish.bat"},
    {"id": "fridge", "name": "冷蔵庫ドア (500x1000)", "param": "?panel=fridge", "file": "setup_led_fridge.bat"},
    {"id": "endcap", "name": "エンド上部 (900x200)", "param": "?panel=endcap", "file": "setup_led_endcap.bat"},
    {"id": "shelf", "name": "棚割り・定番棚 (900x100)", "param": "?panel=shelf", "file": "setup_led_shelf.bat"}
]

for cfg in configs:
    filepath = os.path.join('c:/Users/one/Desktop/RetailMedia_System', cfg["file"])
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(bat_template.replace('{name}', cfg['name']).replace('{param}', cfg['param']).replace('{id}', cfg['id']))
    print(f"Created {cfg['file']}")
