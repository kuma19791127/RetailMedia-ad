...
with open('creator_portal.html', 'r', encoding='utf-8') as f:
    html = f.read()

# Replace the preConfirm and the fetch inside then
...
html = re.sub(r'preConfirm:\s*\(\)\s*=>\s*\{[\s\S]*?\}\s*\}\s*\}\s*\)\.then\(async \(result\)\s*=>\s*\{[\s\S]*?\}\);\s*\}\);\s*\}', r'''preConfirm: async () => {
                ...
                if (result.isConfirmed) {
                    ...
                    fetch(`${API_BASE}/api/creator/bank`, {
                        ...
                        if (res.ok && resData.success) {
                            Swal.fire('保存しました', resData.message || '口座情報が更新されました。', 'success');
                        } else {
                            Swal.fire('エラー', resData.error || '登録に失敗しました。', 'error');
                        }
                    }).catch(e => {
                        console.error(e);
...
with open('creator_portal.html', 'w', encoding='utf-8') as f:
    f.write(html)
...
js = re.sub(r"app\.post\('/api/creator/bank', \(req, res\) => \{[\s\S]*?res\.json\(\{ success: true \}\);\s*\}\);", r"""app.post('/api/creator/bank', async (req, res) => {
